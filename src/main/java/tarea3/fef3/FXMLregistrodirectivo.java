...
public class FXMLregistrodirectivo {
    ...
    public void initialize() {
        ...
        cargarDirectivosEnTabla();
    }

    private void cargarDirectivosEnTabla() {
        tablaDirectivos.getItems().clear();
        tablaDirectivos.getItems().addAll(BD.directivos);
    }
    ...
    public void acc_btnguardar(Event event) {
        try {
            if (fun_validar()) {
                ...
                Directivo objDirectivo = new Directivo(ci, nombres, cargo, equipo);
                BD.directivos.add(objDirectivo);
    ...
    public void acc_cboequipo(ActionEvent actionEvent) {
        try {
            if (cbo_equipolist.getValue() == null) {
                ...
                String equipo = (String) this.cbo_equipolist.getValue();
                tablaDirectivos.getItems().setAll(
                        BD.directivos.stream()
                                .filter(d -> d.getEquipo().equals(equipo))
                                .collect(Collectors.toList())
                );
            }
        } catch (Exception e) {
